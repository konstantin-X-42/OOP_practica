...
# Декораторы @classmethod и @staticmethod меняют поведение метода класса. Относится строго к одному методу,
# который написан сразу же под ним. Они позволяют вызывать методы напрямую через имя класса,
# вообще не создавая конкретный объект (экземпляр) через __init__.
